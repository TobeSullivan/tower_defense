...
**Assume the user has deployed the Nakama stack** (`deploy/nakama/`) to the Hetzner box (`5.78.110.182`, CPX31 @ `hil`) and it's live: client API at `:7350`, console at `:7351` (tunnel-only — `ssh -L 7351:localhost:7351 root@5.78.110.182` → `http://localhost:7351`), server key set in `.env`, the 5 campaign + `ranked_s1` + 60 `trials_*` boards visible in the console. **First thing: confirm it's actually up** before building — ask the user for the deploy log / console screenshot, or have them run `docker compose exec nakama /nakama/nakama healthcheck`. If it's NOT deployed, the steps are in `deploy/nakama/README.md` §1–2 (the box itself is already provisioned + firewalled per §0).
...
### ▶ Nakama meta backend — IN PROGRESS 2026-06-08 (stack authored; box provisioned @ hil; deploy is the immediate next action)
...
- **✅ USER provisioned the box (2026-06-08):** fresh CPX31 created at `hil` (`5.78.110.182`), firewall `firewall-1` set to 3 inbound rules (TCP 22, TCP 7350, UDP 8771 — all any-IP; console is SSH-tunnel-only, no public 7351). Did **not** rescale — CPX11 is deprecated Gen1 and `ash` had no CPX31 capacity; fresh create + redeploy was cleaner since nothing was wired yet. Old box deleted. **▶ IMMEDIATE NEXT (USER, then CC):** `curl get.docker.com | sh` on the box → `rsync deploy/nakama/` up → set `.env` → `docker compose up -d` → verify boards in the console (via tunnel). Then **CC builds** (against the live instance so it's testable): the nakama-godot client + device auth (`NakamaClient` autoload) → a **`NakamaBackend`** for `LeaderboardService` (reads via list/around-owner, writes via `submit_score`) → `set_backend()` it and the 3 leaderboard surfaces light up with real data → then **matchmaking** (ticket→forming-lobby→hand off to the Godot server, which then needs concurrent-match support).
