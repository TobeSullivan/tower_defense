...
1. **Shrink the right rail** — at the 2× touch UI scale it's much too large (user). Multiple fronts: a **sliding drawer for tower details** (instead of the always-docked inspector that overflows + makes Sell unreachable), move Start/Speed/Pause to a compact **bottom strip**, and use the **`art/` UI element set** to make it look polished (BTD6-grade).
2. **Test the 20×11 board on DESKTOP** — confirm it doesn't look too small/chunky scaled up on a monitor; tune if needed. (Generated maps are 20×11 on PC too now.)
